...
def crossval_thresholds(df, cci_col, regime_col, n_splits=5):
    """5-fold cross-validation for CCI-only threshold classifier."""
    ...
    for train_idx, test_idx in kf.split(df):
        ...
        best = grid_search_thresholds(
            train[cci_col].values,
            train[regime_col].values,
            n_grid=200
        )

        preds = [
            classify_threshold(v, best["t1"], best["t2"])
            for v in test[cci_col]
        ]

        acc = accuracy_score(test[regime_col], preds)
        accs.append(acc)
...
# =========================================================
# CROSS-VALIDATION (CCI-only)
# =========================================================
cv_accs = crossval_thresholds(df, cci_col, regime_col, n_splits=5)
cv_mean = np.mean(cv_accs)
cv_std  = np.std(cv_accs, ddof=1) if len(cv_accs) > 1 else 0.0

print("\n" + "=" * 80)
print("5-FOLD CROSS-VALIDATION (CCI-only)")
print(f"  fold accuracies = {[round(a,6) for a in cv_accs]}")
...
with open(summary_path, "w", encoding="utf-8") as f:
    ...
    f.write("Cross-validation (5-fold, CCI-only):\n")
    f.write(f"  accuracies = {[round(a,6) for a in cv_accs]}\n")
    f.write(f"  mean = {cv_mean:.6f}\n")
    f.write(f"  std  = {cv_std:.6f}\n\n")
