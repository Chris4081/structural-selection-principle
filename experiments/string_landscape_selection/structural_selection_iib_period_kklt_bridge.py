#!/usr/bin/env python3
"""Period-data-to-KKLT bridge test for structural selection.

This script replaces the direct W0 proxy of the IIB/KKLT bridge by a concrete
large-complex-structure period calculation. Integer flux vectors F,H are paired
with a one-parameter Calabi-Yau period vector Pi(t), and the effective flux
superpotential is computed as

    W0_raw = (F - tau_ax H) dot Pi(t).

The resulting |W0_raw| is converted into the small real W0 used by the reduced
KKLT solver. This remains a controlled LCS benchmark, not a full period-database
compactification scan.
"""
...
class PeriodBridgeConfig:
    seed: int = 2704
    n_backgrounds: int = 120
    flux_dim: int = 4
    flux_abs_max: int = 6
...
def lcs_period_vector(t_modulus: complex, config: PeriodBridgeConfig) -> np.ndarray:
    """One-parameter large-complex-structure period vector.

    This is a compact benchmark period basis, not an exact instanton-corrected
    mirror-quintic period computation.
    """
    kappa = config.kappa
    c2 = config.c2_dot_j
    chi = config.chi
    zeta_term = 1j * chi * ZETA_3 / (2.0 * (2.0 * math.pi) ** 3)
    return np.array(
        [
            1.0 + 0j,
            t_modulus,
            0.5 * kappa * t_modulus**2 + c2 / 24.0,
            -(kappa / 6.0) * t_modulus**3 - (c2 / 24.0) * t_modulus + zeta_term,
        ],
        dtype=complex,
    )
...
def w0_from_period_data(
    f_flux: np.ndarray,
    h_flux: np.ndarray,
    t_modulus: complex,
    tau_ax: complex,
    config: PeriodBridgeConfig,
) -> tuple[complex, float]:
    periods = lcs_period_vector(t_modulus, config)
...
def make_background(idx: int, rng: np.random.Generator, config: PeriodBridgeConfig) -> dict:
    ...
    x_value = float(rng.uniform(-0.5, 0.5))
    y_value = float(rng.uniform(config.y_min, config.y_max))
    t_modulus = complex(x_value, y_value)
    ...
    w_raw, w0 = w0_from_period_data(f_flux, h_flux, t_modulus, tau_ax, config)
    flux_norm = float(np.dot(f_flux, f_flux) + np.dot(h_flux, h_flux))
    max_norm = float(2 * config.flux_dim * config.flux_abs_max**2)
    d_uplift = config.d_uplift_max * (0.35 + 0.65 * flux_norm / max_norm) * (gs / config.gs_max)
    period_control = (1.0 / y_value) ** 2 + gs**2
    ...
    return {
        ...
        "W0": w0,
        "D": float(d_uplift),
        "period_control_defect": float(period_control),
    }


def period_bridge_record(background: dict, kklt_record: dict, config: PeriodBridgeConfig) -> dict:
    ...
    delta_dim = (10.0 / tau) ** 2 + math.exp(-config.a * tau) + 0.35 * background["period_control_defect"]
    ...
    f_period_bridge = -math.log(config.epsilon + period_support)
    f_sw = 12.0 * delta_q_10**2 + 3.0 * max(0.0, 1.25 - background["t_imag"]) ** 2 + 2.0 * tachyon_defect
    lambda_penalty = 0.5 * ((energy - config.lambda_obs) / config.lambda_sigma) ** 2
    f_pheno = f_period_bridge + f_sw + lambda_penalty
    ...
    return {
        **background,
        ...
        "period_bridge_defects": {
            "d_B10": d_b10,
            "Delta_Q_10": delta_q_10,
            "Delta_E_KKLT": delta_e_kklt,
            "Delta_dim_period_KKLT": delta_dim,
            "period_control_defect": background["period_control_defect"],
            "tachyon_defect": tachyon_defect,
        },
        "period_bridge_support": period_support,
        "F_period_bridge": f_period_bridge,
        "F_SW_period": f_sw,
        "lambda_penalty": lambda_penalty,
        "F_pheno_period": f_pheno,
    }


def generate_scan(config: PeriodBridgeConfig) -> tuple[list[dict], list[dict]]:
    ...
    while len(backgrounds) < config.n_backgrounds and attempts < config.n_backgrounds * 120:
        attempts += 1
        background = make_background(len(backgrounds), rng, config)
        key = tuple(background["F_flux"] + background["H_flux"] + [round(background["t_imag"] * 1000)])
        if key in seen:
            continue
        seen.add(key)
        ...
        for tau, theta in points:
            ...
            candidates.append(period_bridge_record(background, kklt, config))

    if not candidates:
        raise RuntimeError("No period-to-KKLT stationary candidates found.")

    ranks = {
        "rank_energy": rankdata(item["abs_energy"] for item in candidates),
        "rank_period_bridge": rankdata(item["F_period_bridge"] for item in candidates),
        "rank_pheno_period": rankdata(item["F_pheno_period"] for item in candidates),
...
def top_summary(candidates: list[dict], key: str, top_n: int = 12) -> dict:
    ...
    return {
        ...
        "mean_t_imag": float(np.mean([item["t_imag"] for item in top])),
...
def summarize(backgrounds: list[dict], candidates: list[dict], config: PeriodBridgeConfig) -> dict:
    top_energy = set(top_summary(candidates, "abs_energy")["ids"])
    top_period = set(top_summary(candidates, "F_period_bridge")["ids"])
    top_pheno = set(top_summary(candidates, "F_pheno_period")["ids"])
    ...
    return {
        ...
        "spearman_abs_energy_vs_F_period_bridge": spearman(
            [item["abs_energy"] for item in candidates],
            [item["F_period_bridge"] for item in candidates],
        ),
        "spearman_abs_delta_q_vs_abs_energy": spearman(
            [item["abs_Delta_Q_D3"] for item in candidates],
            [item["abs_energy"] for item in candidates],
        ),
        "spearman_abs_delta_q_vs_F_period_bridge": spearman(
            [item["abs_Delta_Q_D3"] for item in candidates],
            [item["F_period_bridge"] for item in candidates],
        ),
        "spearman_period_control_vs_F_period_bridge": spearman(
        ...
        "top_period_bridge": top_summary(candidates, "F_period_bridge"),
        "top_pheno_period": top_summary(candidates, "F_pheno_period"),
        "top12_overlap_energy_period_bridge": len(top_energy & top_period),
        "top12_overlap_energy_pheno_period": len(top_energy & top_pheno),
        "best_energy_candidate": min(candidates, key=lambda item: item["abs_energy"]),
        "best_period_bridge_candidate": min(candidates, key=lambda item: item["F_period_bridge"]),
        "best_pheno_period_candidate": min(candidates, key=lambda item: item["F_pheno_period"]),
    }


def plot_energy_period(candidates: list[dict], output: Path) -> None:
    ...
    for item in sorted(candidates, key=lambda obj: obj["F_period_bridge"])[:5]:
        ax.annotate(
            f"id={item['background_id']}\n|dQ|={item['abs_Delta_Q_D3']}\ny={item['t_imag']:.2f}",
            (item["abs_energy"], item["F_period_bridge"]),
    ...
    ax.set_ylabel(r"$F_{\mathrm{period}}^{(10)}$")
    ax.set_title("Period-to-KKLT Bridge: Energy Proximity Versus Structural Score")
...
def plot_rank_period(candidates: list[dict], output: Path) -> None:
    fig, ax = plt.subplots(figsize=(6.8, 6.0))
    scatter = ax.scatter(
        [item["rank_energy"] for item in candidates],
        [item["rank_period_bridge"] for item in candidates],
        c=[item["t_imag"] for item in candidates],
        cmap="magma",
        s=50,
        alpha=0.82,
    )
    ...
    ax.set_ylabel(r"Rank by $F_{\mathrm{period}}^{(10)}$")
    ax.set_title("Period-Bridge Ranking Versus Energy Ranking")
    ax.grid(True, alpha=0.25)
    cbar = fig.colorbar(scatter, ax=ax)
    cbar.set_label(r"LCS modulus $\mathrm{Im}(t)$")
    fig.tight_layout()
    fig.savefig(output, dpi=220)
    plt.close(fig)


def plot_w0_period(candidates: list[dict], output: Path) -> None:
    fig, ax = plt.subplots(figsize=(8.0, 5.2))
    scatter = ax.scatter(
        [item["W0_raw_abs"] for item in candidates],
        [abs(item["W0"]) for item in candidates],
        c=[item["g_s"] for item in candidates],
        cmap="cividis",
        s=48,
        alpha=0.84,
    )
    ax.set_xlabel(r"$|W_{0,\mathrm{raw}}|=|(F-\tau_{\mathrm{ax}}H)\cdot\Pi(t)|$")
    ax.set_ylabel(r"Reduced KKLT $|W_0|$")
    ax.set_title("Period Data Generates the Effective Flux Superpotential")
    ax.grid(True, alpha=0.25)
    cbar = fig.colorbar(scatter, ax=ax)
    cbar.set_label(r"$g_s$")
    fig.tight_layout()
    fig.savefig(output, dpi=220)
    plt.close(fig)


def plot_period_decomposition(candidates: list[dict], output: Path, top_n: int = 8) -> None:
    top = sorted(candidates, key=lambda item: item["F_period_bridge"])[:top_n]
    ...
    parts = [
        ...
        ("Delta_dim_period_KKLT", r"$\Delta_{\mathrm{dim+period}}$"),
    ...
    for color, (key, label) in zip(colors, parts):
        values = np.array([item["period_bridge_defects"][key] for item in top], dtype=float)
        ax.bar(labels, values, bottom=bottom, color=color, label=label, alpha=0.9)
        bottom += values
    ax.set_ylabel("Period-bridge denominator contribution")
    ax.set_title("Top Period-to-KKLT Candidates: Obstruction Decomposition")
...
def make_plots(candidates: list[dict], output_dir: Path) -> list[str]:
    ...
    plots = {
        "energy_vs_period_bridge": output_dir / "energy_vs_period_bridge.png",
        "rank_period_bridge": output_dir / "rank_period_bridge.png",
        "w0_period_map": output_dir / "w0_period_map.png",
        "period_obstruction_decomposition": output_dir / "period_obstruction_decomposition.png",
    }
    plot_energy_period(candidates, plots["energy_vs_period_bridge"])
    plot_rank_period(candidates, plots["rank_period_bridge"])
    plot_w0_period(candidates, plots["w0_period_map"])
    plot_period_decomposition(candidates, plots["period_obstruction_decomposition"])
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("structural_selection_iib_period_kklt_bridge_results.json"))
    parser.add_argument("--plot-dir", type=Path, default=Path("structural_selection_iib_period_kklt_bridge_plots"))
    args = parser.parse_args()

    config = PeriodBridgeConfig(seed=args.seed, n_backgrounds=args.n_backgrounds)
    ...
    payload = {
        "description": "Period-data-to-KKLT bridge scan using one-parameter LCS periods.",
        "summary": summary,
        "plots": plots,
