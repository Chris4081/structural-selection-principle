#!/usr/bin/env python3
"""Stage the Paper 73 SATLIB/DIMACS dataset.

This is a thin Paper-73 wrapper around the Paper-69 SATLIB stager. It changes
only the local output folder, manifest path, default extraction limit, and
user-agent label. Raw CNFs remain under data_external/ and must not be committed.
"""
...
def download_archive(archive: SatlibArchive, archive_dir: Path, allow_insecure_ssl: bool) -> Path:
    ...
    request = urllib.request.Request(archive.url, headers={"User-Agent": "MAAT-Paper73-SATLIB-Stager/1.0"})
    ...
    try:
        ...
        if "CERTIFICATE_VERIFY_FAILED" in str(exc) and not allow_insecure_ssl:
            raise RuntimeError(
                ...
                "--allow-insecure-ssl. The manifest will record this exception "
                "and every extracted CNF will still be fixed by SHA256."
            ) from exc
        raise
    return out
...
def main() -> None:
    ...
    parser.add_argument("--archives", nargs="+", default=DEFAULT_ARCHIVES, choices=sorted(ARCHIVES))
    parser.add_argument(
        "--limit-per-archive",
        type=int,
        default=10,
        help="Paper-72 primary rule: stage 10 CNFs per family where available.",
    )
    parser.add_argument("--manifest", type=Path, default=MANIFEST)
    parser.add_argument(
        "--allow-insecure-ssl",
        action="store_true",
        help="Opt-in only: disable TLS verification for legacy SATLIB downloads; all files remain SHA256-fixed.",
    )
